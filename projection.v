...
Fixpoint marked_heap_lookup (n:nat)(hp:list ((LowLang.tm*Ty)*(nat*nat))):(option (LowLang.tm*Ty)):=
  ...
  | h::t , S n' =>marked_heap_lookup n' t
  end.

(*extract the result of [heap_lookup]*)
Definition marked_efst (p:option(LowLang.tm*Ty)) : LowLang.tm :=
 match p with
 | None => LowLang.tvar (Id 100)
 | Some (t , T) => t
 end.
...
return_smallest_match (length hp)(project_hp hp) = (true,(0,0)).
Proof. Admitted.
...
n <> n1 ->
return_smallest_match n ((L,(n1,n2)) :: nil)= (true,(0,0)).
...
Lemma try:forall t n n1 n2 hp,
t = SecLang.efst (SecLang.heap_lookup n hp) ->
false = fst (return_smallest_match n (marked_heap (marked_heap' hp n1) n2)) ->
project_e t =
marked_efst (marked_heap_lookup (snd (snd (return_smallest_match n (marked_heap (marked_heap' hp n1) n2)))) (marked_heap (marked_heap' hp n1) n2)).
Proof. intros. generalize dependent t. generalize dependent n. generalize dependent n1. generalize dependent n2. induction hp. 
Case ("nil"). intros. simpl in H1. inversion H1.
Case ("h::t"). intros. destruct a.  simpl. destruct n1. simpl. remember (SecLang.label t0) as BB. destruct BB. simpl. destruct n. simpl. simpl in H0. subst. reflexivity. simpl in H1. rewrite<-HeqBB in H1. simpl in H1. simpl in H0.
              assert (marked_heap_lookup (snd (snd (return_smallest_match (S n)(marked_heap (marked_heap' hp 1) n2))))((project_e t0,t1,(0,0)) :: marked_heap (marked_heap' hp 1) n2) = marked_heap_lookup (snd (snd (return_smallest_match n (marked_heap (marked_heap' hp 0) n2))))(marked_heap (marked_heap' hp 0) n2)). admit.
              rewrite->H2. clear H2. apply IHhp. admit. apply H0.
              simpl in H1. rewrite<-HeqBB in H1. destruct n. (*contradiction*) admit. simpl in H0.
              assert (marked_heap_lookup (snd (snd (return_smallest_match (S n)(marked_heap (marked_heap' hp 1) (S n2)))))(marked_heap (marked_heap' hp 1) (S n2)) = marked_heap_lookup (snd (snd (return_smallest_match n (marked_heap (marked_heap' hp 0) (S n2)))))(marked_heap (marked_heap' hp 0) (S n2))). admit.
              rewrite->H2. clear H2. apply IHhp. admit. apply H0.

              destruct n. (*contradiction*) admit. simpl. destruct n2. remember (SecLang.label t0) as BB. destruct BB. simpl. remember (beq_nat n1 n) as CC. destruct CC. simpl. simpl in H1. rewrite<-HeqBB in H1. simpl in H1. rewrite<-HeqCC in H1. simpl in H0.


              simpl. remember (SecLang.label t0) as BB. destruct BB. destruct n2. simpl in H1. rewrite<-HeqBB in H1. destruct n. (*contradiction*) admit. simpl. remember (beq_nat n1 n) as CC. destruct CC. simpl. 
               



 simpl in H1. rewrite<-HeqBB in H1. simpl in H1. 
              assert (fst (return_smallest_match (S n)(marked_heap (marked_heap' hp 1) n2)) = fst (return_smallest_match n (marked_heap (marked_heap' hp 0) n2))). admit.
              rewrite<-H2. clear H2. apply H1. simpl in H0. apply H0. simpl in H1. rewrite<-HeqBB in H1. apply IHhp. apply H1. 
              (*we are stuck again*)admit.
        

(**
Lemma try:forall n n' n1 t hp,
n < length hp ->
t = SecLang.efst (SecLang.heap_lookup n hp) ->
(false,(n,n')) = return_smallest_match n (marked_heap (marked_heap' hp n1) 0) ->
project_conf'_e (project_e t)(marked_heap (marked_heap' hp n1) 0)
=LowLang.efst (LowLang.heap_lookup
  n'
 (erase_hp (project_conf'_hp (marked_heap (marked_heap' hp n1) 0)(marked_heap (marked_heap' hp n1) 0)))
).
Proof. intros. generalize dependent n. generalize dependent n'. generalize dependent n1. generalize dependent t.
induction hp.
Case ("nil"). intros. simpl in H0. destruct n. apply LowLang.lt_same_F in H0. inversion H0. inversion H0.
Case ("h::t"). intros. destruct a. simpl. destruct n1. simpl. remember (SecLang.label t0) as BB. destruct BB. 
               simpl. destruct n'. simpl. (*prove t=t0*) admit.
               simpl. assert (project_conf'_e (project_e t)((project_e t0, t1, (0, 0)) :: marked_heap (marked_heap' hp 1) 0)=project_conf'_e (project_e t)(marked_heap (marked_heap' hp 1) 0)). admit.
               rewrite->H3. clear H3. assert (project_conf'_hp (marked_heap (marked_heap' hp 1) 0)((project_e t0, t1, (0, 0)) :: marked_heap (marked_heap' hp 1) 0)=project_conf'_hp (marked_heap (marked_heap' hp 1) 0)(marked_heap (marked_heap' hp 1) 0)). admit.
               rewrite->H3. clear H3. 
*)
...
Case ("st_derefloc").
               ...
               erase_hp (project_conf'_hp (project_hp hp0) (project_hp hp0)))). apply LowLang.st_derefloctH. apply LowLang.Multi_refl. symmetry. apply HeqBB. 
...
Case ("st_assign").
 ...
 (**
                unfold project. destruct b. simpl. unfold project_conf. simpl. remember (fst(return_smallest_match n (project_hp hp0))) as BB. destruct BB. apply return_smallest_match_project_e_true in H0.
                rewrite<-H1 in H0. rewrite->H0. simpl. apply LowLang.Multi_step with (y:=(LowLang.tH,erase_hp (project_hp hp0))). apply LowLang.st_derefloctH. rewrite->erase_hp_length. reflexivity. apply LowLang.Multi_refl.
                symmetry. apply HeqBB. simpl. apply LowLang.Multi_step with (y:=(fst (project_conf' (project_e t)(project_hp hp0)),erase_hp (project_hp hp0))). apply LowLang.st_derefloc. admit. 
*)
